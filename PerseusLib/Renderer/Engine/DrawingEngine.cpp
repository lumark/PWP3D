...
void DrawingEngine::Draw(Object3D* object, View3D* view, Pose3D *pose, ImageUChar *imageRender, RenderingType renderingType, bool clearImage)
{
	...
	if (renderingType == RENDERING_FILL) drawFilled(imageRender, renderObject->drawingModel[view->viewId], object->objectId);
	else drawWireframe(imageRender, renderObject->drawingModel[view->viewId], roi);
}
